class CaesarCipher(shift:Int) {
  ...
  def encryptText(text: String): String = {
    ...
    for (c <- text) {
      ...
      else if (c == ' ') msg += ' ' //space does not need to be encrypted
  ...
  def decryptText(cryptText: String): String = {
    ...
    for (c <- cryptText) {
      if (alphabet.contains(c)) { //at lowercase char
        ...
        var index: Int = (alphabet(c.toLower) - shift) % 26
        //Scala modulo will give a negative result if dividend is negative
        if (index < 0) {
          index = 26 + index
        }
        msg += alphabet.find(_._2 == index).map(_._1).get.toUpper
